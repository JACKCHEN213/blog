	...
	<div class="result_wrap">
		<div class="result_content">
			<table class="list_tab">
				...
				<tr>
					<td class="tc"><input type="text" onchange="changeOrder(this, {{$value->nav_id}})" value="{{$value->nav_order}}" v-bind:width="10px"></td>
					<td class="tc">{{$value->nav_id}}</td>
					<td class="tc"><a class="tc" href="{{$value->nav_url}}" target="_blank">{{$value->nav_name}}</a></td>
					<td class="tc">{{$value->nav_alias}}</td>
					<td class="tc tc_addr">{{$value->nav_url}}</td>
					<td class="tc tc_time">{{$value->updated_at}}</td>
					<td class="tc">
						<a href="{{url('admin/navs/'.$value->nav_id.'/edit')}}">修改</a>
						<a href="javascript:;" onclick="navDel({{$value->nav_id}})">删除</a>
					</td>
				</tr>
				@endforeach
			</table>
...
<script>
	...
	function navDel(nav_id) {
        layer.confirm('您确定要删除这个导航？', {
            btn: ['确定','取消'] //按钮
        }, function(){
